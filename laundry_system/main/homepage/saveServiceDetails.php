...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pickupFee = $_POST['pickup_fee'] ?? null;
    ...
    foreach ($requestIds as $requestId) {
        ...
        $sqlTransaction = "INSERT INTO transaction (request_id, customer_id, service_option_id, service_option_name, laundry_cycle, customer_address, total_amount, delivery_fee, pickup_fee, rush_fee, amount_tendered, money_change)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmtTransaction = $conn->prepare($sqlTransaction);
        $stmtTransaction->bind_param('iiissssddddd', $requestId, $customerId, $serviceId, $serviceOptionName, $isRush, $address, $totalAmount, $deliveryFee, $pickupFee, $rushFee, $amountTendered, $change);
        ...
        // Update order status to 'completed' for the current request_id
